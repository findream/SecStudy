...
DECLSPEC_IMPORT int __cdecl MSVCRT$memcmp(const void *str1, const void *str2, size_t n);
...
DECLSPEC_IMPORT errno_t __cdecl MSVCRT$mbstowcs_s(size_t* pReturnValue, wchar_t* wcstr, size_t sizeInWords, const char* mbstr, size_t count);


DECLSPEC_IMPORT HRESULT WINAPI MSCOREE$CLRCreateInstance(REFCLSID clsid, REFIID riid, LPVOID* ppInterface);


DECLSPEC_IMPORT LPWSTR* WINAPI SHELL32$CommandLineToArgvW(LPCWSTR lpCmdLine, int* pNumArgs);
